...
%\usepackage[paperheight=24cm,paperwidth=17cm,margin=1.5cm,inner=2cm,outer=1.5cm]{geometry}
%
%\usepackage[
%	% set width and height to a4 width and height + 6mm
%	%width=16truecm, height=24truecm,
%	a4,
...
% adjust margins (should be done before importing fancyhdr!) %lmargin=2.5cm,rmargin=4cm,tmargin=3cm,bmargin=3cm
...
% magic for reading parts of a file
\makeatletter
\newread\pin@file
\newcounter{pinlineno}
\newcommand\pin@accu{}
...
\newcommand{\doack}{%
%	{\footnotesize %
%    %\#\theacki\ | % LINENUMBER DEBUG
%		\partialinput{\theacki}{\theacki}{ack}
%	}%
	\stepcounter{acki}%
}
...
\fancypagestyle{mainmatter}{%    
	...
	\fancyfoot[RO,LE]{\textsf{{\footnotesize Page \thepage}}}
	\fancyfoot[LO]{\ifodd\theacki\doack\fi}
	\fancyfoot[RE]{\ifodd\theacki\else\doack\fi}
...
\titleformat{\chapter}[hang]{\LARGE\bfseries}{\thechapter\hsp\textcolor{light-gray}{|}\hsp}{0pt}{\LARGE\bfseries}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage{\thispagestyle{empty}\cleardoublepage}
\chapter{dyngen: Benchmarking with \textit{in silico} single cells} 
\chaptermark{dyngen: Benchmarking with \textit{in silico} single cells.}
\label{chap:dyngen}
...
\chaptermark{dynbenchmark: A comparison of single-cell trajectory inference methods.}
...
{\large \textbf{Abstract}}
\begin{quote}
	\textbf{Purpose:} Graphlets are small network patterns that can be counted in order to characterise the structure of a network (topology). Counting graphlet can used to iteratively optimise a network in order to achieve certain topological properties.
