...
cran <- c(
  ...
  "qsub", "SCORPIUS", "badger", "devtools", "mlr", "ParamHelpers", "remotes", "rlang"
)
...
summ <- dls %>% filter("2019-08-01" <= start) %>% group_by(package) %>% summarise(downloads = sum(downloads) / num_days * 365)
summ %>% print(n = 50)
