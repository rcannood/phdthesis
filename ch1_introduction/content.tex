...
Over the past five years, already 450 new tools for analysing single-cell omics data have been developed\cite{zappia_exploringsinglecellrnaseq_2018}. These tools typically fall within one of seven categories (Figure \ref{fig:comp_tools}). We discuss each category in the following subsections.
...
\begin{figure}[htb!]
	\centering
	\includegraphics[width=\linewidth]{fig/comptools/comptools_4.pdf} 
	\caption{
		\textbf{Computational tools developed for processing single-cell omics data usually lie within one of seven categories.}
		\textbf{A:} Example of a count matrix from a scRNA-seq dataset, showing only 100 genes (rows) and 100 cells (columns).
		\textbf{B:} Dimensionality reduction, providing a visual overview of the main cellular heterogeneity in the dataset.
		\textbf{C:} Clustering, grouping together cells with similar omics profiles.
		\textbf{D:} Trajectory inference, identifying and characterising transitions between different cellular states.
		\textbf{E:} Differential expression, \ldots.
		\textbf{F:} Network inference, predicting gene regulatory interactions from cell-to-cell heterogeneity.
		\textbf{G:} RNA velocity, inferring the directionality of trajectories by predicting the future state of each cell.
		\textbf{H:} Trajectory alignment, aligning multiple trajectories.
	}
