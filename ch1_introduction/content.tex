...
\comment{use this reference? \cite{cornell_prebioticaminoacids_2019}}
...
\comment{HT: feels like an enumeration, should make it more into a story}
...
\comment{do not only discuss differentiation, but instead any kind of developmental process; and mention 'developmental trajectory'.}
...
\comment{are dynamic processes defined? a dynamic process is just a biochemical reaction}
...
\comment{TODO: download database and calculate some statistics?}
...
\comment{TODO: explain regulation mechanisms such as transcription factor binding sites?}
\subsection{Profiling single cells}
\comment{This sections goes greatly into detail about IHC and cytometry. Can the merging of the 'single-cell' and 'omics' be explained without going so much into detail?}
...
\comment{Rephrase the microscope sentence}
...
\comment{Demonstrate the masking effect of bulk analyses.}
...
\comment{Make a better connection to the subsections below}
...
\comment{This section is not very interestingly written}
...
\comment{Could still expand this section with pieces from the EJI paper, though it needs to be adapted strongly.}
...
\comment{Get a better focus for the overall contributions of the thesis. For example, focus on datasets that contain developing cells, and that the tools we develop make this assumption.}
