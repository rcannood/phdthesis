...
At the moment, only a subset of the dynverse packages are present on CRAN: dynutils, babelwhale, dynparam and dyndimred. We are currently in the process of submitting the next packages in our dependency graph on CRAN as well: dynwrap and dynplot. In this work package, we will submit all other dynverse packages as well to CRAN, so that the dynverse toolkit becomes broadly distributed. This includes making sure the package code complies with CRAN policy, adding extensive documentation and examples, and updating the codebase based on the comments from the CRAN maintainers.

Furthermore, the work packages also includes maintaining and extending existing software packages on CRAN or Bioconductor on which our packages depend. We already have extensive experience with contributing to our dependencies. This includes speeding up parts of the dependency (princurve, proxyC), fixing bugs (monocle, splatter, slingshot, rlang, devtools), becoming a maintainer of orphaned packages (princurve, GillespieSSA), extending the documentation (devtools) and adding new functionality (ParamHelpers). We will keep doing this within this work package, with a focus on dependencies that help with plotting (ggplot2, ggraph, patchwork) and trajectory inference (monocle, slingshot, PAGA).

\paragraph{Author of the following packages:}
\ \\
{
	\small
	\begin{tabular}{llp{10cm}}
		babelwhale    & CRAN   & Interacting with Docker and Singularity containers                        \\
		dyndimred     & CRAN   & Applying dimensionality reduction methods                                 \\
		dyneval       & Github & Evaluating trajectory inference methods                                   \\
		dynfeature    & Github & Calculating feature importance scores from trajectories                   \\
		dyngen        & Github & Simulating single-cell data using gene regulatory networks                \\
		dynguidelines & Github & User guidelines for trajectory inference                                  \\
		dynmethods    & Github & A collection of wrappers for trajectory inference methods                 \\
		dyno          & Github & A pipeline for inferring, visualising and interpreting trajectories       \\
		dynparam      & Github & Creating meta-information for parameters                                  \\
		dynplot       & Github & A simple visualisation library for trajectories                           \\
		dynplot2      & Github & A fully customisable visualisation library for trajectories               \\
		dyntoy        & Github & Generating simple toy data of cellular differentiation                    \\
		dynutils      & CRAN   & Common functionality for the dynverse packages                            \\
		dynwrap       & Github & A common format for trajectories                                          \\
		GillespieSSA  & CRAN   & Gillespie's Stochastic Simulation Algorithm (SSA)                         \\
		GillespieSSA2 & CRAN   & Gillespie's Stochastic Simulation Algorithm for Impatient People          \\
		gng           & Github & An Rcpp implementation of the Growing Neural Gas algorithm                \\
		incgraph      & CRAN   & Incremental graphlet counting for network optimisation                    \\
		princurve     & CRAN   & Fits a principal curve in arbitrary dimension                             \\
		proxyC        & CRAN   & Computes proximity in large sparse matrices                               \\
		qsub          & CRAN   & Running commands remotely on gridengine clusters                          \\
		SCORPIUS      & CRAN   & Inferring developmental chronologies from single-cell RNA sequencing data
	\end{tabular}
}
...
\ \\
{
	\small
	\begin{tabular}{llp{10cm}}
		ClusterSignificance & Bioconductor & Assess if class clusters in dimensionality reduced data representations have a separation different from permuted data                                           \\
		devtools            & CRAN         & Tools to make developing R packages easier                                                                                                                       \\
		merlot              & Github       & A method for reconstructing lineage-tree topologies from scRNA-seq data                                                                                          \\
		mlr                 & CRAN         & Machine Learning in R                                                                                                                                            \\
		monocle             & Bioconductor & Clustering, differential expression, and trajectory analysis for single-cell RNA-Seq                                                                             \\
		ParamHelpers        & CRAN         & Helpers for Parameters in Black-Box Optimization, Tuning and Machine Learning                                                                                    \\
		pseudogp            & Github       & Probabilistic pseudotime for single-cell RNA-seq                                                                                                                 \\
		remotes             & CRAN         & R package installation from remote repositories, including GitHub                                                                                                \\
		rlang               & CRAN         & Functions for base types and core R and tidyverse features                                                                                                       \\
		SCope               & Github       & Visualization of large-scale and high dimensional single cell data                                                                                               \\
		slingshot           & Bioconductor & Tools for ordering single-cell sequencing                                                                                                                        \\
		splatter            & Bioconductor & Simple simulation of single-cell RNA sequencing data                                                                                                             \\
		URD                 & Github       & URD reconstructs transcriptional trajectories underlying specification or differentiation processes in the form of a branching tree from single-cell RNAseq data \\
		wishbone            & Github       & identify bifurcating developmental trajectories from single-cell data
	\end{tabular}
%\endgroup{}
}
